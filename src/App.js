...
import { CubeCamera, Float, MeshReflectorMaterial, useGLTF } from '@react-three/drei'
  ...
  return (
    <Canvas camera={{ position: [0, 0, 30], fov: 35, near: 1, far: 60 }} gl={{ antialias: false }}>
      ...
      <Float rotationIntensity={3} floatIntensity={3} speed={1}>
        <BlackBox position={[-3, -1, -5]} resolution={256} frames={Infinity}>
          {(texture) => (
            <mesh>
...
export function BlackBox(props) {
  const { nodes, materials } = useGLTF('/black6.glb')
  return (
    <group {...props} dispose={null}>
      <group position={[-0.011, 3.925, 0.163]} rotation={[1.675, -0.017, 2.68]} scale={2.217}>
        <mesh geometry={nodes.boxboxgroup3_boxgroup16group13polySurface38.geometry} material={materials.Base_box} position={[-0.057, 0.113, 0.013]} />
      </group>
      <group position={[0.011, 4.718, -0.166]} rotation={[Math.PI / 2, 0, 0]} scale={2.241}>
        <mesh geometry={nodes.black.geometry} material={materials.black_suuri} position={[-0.034, 0.187, 2.108]} rotation={[0, 0, 1.829]} scale={0.7} />
      </group>
    </group>
  )
}
